...
class GroupedSortedMappedEntityCollectionController extends IPEController implements IPEControllerInterface
{
    ...
    public function editAction($ipe_hash, $ipe, Request $request)
    {
        if ($ipe['find_params']['is_collection']) {
            ...
            return $this->collectionListAction($ipe_hash, $ipe, $request);
        }
        else {
            ...
            return $this->collectionEditItemAction($ipe_hash, $ipe, $request);
        }
    }

    public function collectionEditItemAction($ipe_hash, $ipe, Request $request)
    {
            ...
            if ($form->isValid()) {
                ...
                try{
                    ...
                    if ($action_on_success == 'list') {
                        return $this->collectionListAction($ipe_hash, $ipe, $request, true);
                    }
                }
                catch (\Exception $e)
                {
    ...
    public function collectionListAction($ipe_hash, $ipe, Request $request, $reload_content = false)
    {
        $this->checkRoles($ipe['params']['editor_roles']);
    ...
    public function collectionMoveItemAction($ipe_hash, $ipe, Request $request)
    {
        $position = $request->query->get('position');
        $id = $request->query->get('id');
        $this->checkRoles($ipe['params']['editor_roles']);
    ...
    public function collectionRemoveItemAction($ipe_hash, $ipe, Request $request)
    {
        $id = $request->query->get('id');        
        $this->checkRoles($ipe['params']['editor_roles']);
        $params = $ipe['params'];
    ...
    public function collectionAddItemAction($ipe_hash, $ipe, Request $request)
    {
        $this->checkRoles($ipe['params']['editor_roles']);
        $list = $this->getObject($ipe['ipe_definition'], $ipe['find_params'], $ipe['params']);
        $params = $ipe['params'];
